...
from app.repositories.build_recommendation_repository import BuildRecommendationRepository
...
def get_recommended_build(
    ...
    if patch is None:
        patch = repo.get_latest_patch(elo_tier, lane, champion_id)
        if patch is None:
            return None

    row = repo.get_one(patch, elo_tier, lane, champion_id)
    if row is None:
        return None
