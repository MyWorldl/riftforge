...
def list_scores(db: Session, elo_tier: str, lane: str | None, patch: str | None) -> list[dict]:
    ...
    if patch is None:
        patch = score_repo.get_latest_patch(elo_tier)
        if patch is None:
            return []

    rows = score_repo.list_by_patch(elo_tier, patch, lane)
    ...
    perf_by_key = {
        (r.patch, r.tier, r.lane, r.champion_id): r for r in perf_repo.list_by_patch(elo_tier, patch)
    }
    skill_by_key = {
        (r.patch, r.tier, r.lane, r.champion_id): r for r in skill_repo.list_by_patch(elo_tier, patch)
    }

    result = []
    for row in rows:
        key = (row.patch, row.elo_tier, row.lane, row.champion_id)
...
def get_explanation(
    ...
    row = score_repo.get_one(champion_id, lane, elo_tier, patch)
    if row is None:
        return None
...
def list_available_patches(db: Session, elo_tier: str) -> list[str]:
    """Patches com score calculado pro elo, do mais recente pro mais
    antigo — alimenta o filtro de patch da interface."""
    return ChampionScoreRepository(db).list_available_patches(elo_tier)


def get_champion_history(
    db: Session, champion_id: str, elo_tier: str, lane: str | None
) -> list[dict]:
    """Item 3.4: evolução do score de um campeão entre patches."""
    score_repo = ChampionScoreRepository(db)
    perf_repo = PerformanceRepository(db)

    rows = score_repo.get_history(champion_id, elo_tier, lane)
    n_matches_by_key = {
        (r.patch, r.tier, r.lane, r.champion_id): r.n_matches
        for r in perf_repo.list_by_champion(elo_tier, champion_id, lane)
    }
