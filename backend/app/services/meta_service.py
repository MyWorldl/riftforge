...
def get_coverage(db: Session, elo_tier: str, patch: str | None, region: str) -> dict:
    ...
    resolved_patch = patch or repo.get_latest_patch(elo_tier, region)
    if resolved_patch is None:
        return {"patch": None, "elo_tier": elo_tier, "cobertura": []}

    rows = repo.list_coverage_by_lane(elo_tier, resolved_patch, region)
    return {
        "patch": resolved_patch,
        "elo_tier": elo_tier,
        "cobertura": [
            {"lane": lane, "cobertura": cobertura} for lane, cobertura in rows
        ],
    }
