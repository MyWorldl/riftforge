...
class ChampionScoreRepository:
    ...
    def get_latest_patch(self, elo_tier: str) -> str | None:
        row = (
    ...
    def list_by_patch(self, elo_tier: str, patch: str, lane: str | None = None) -> list[ChampionScore]:
        query = self.db.query(ChampionScore).filter_by(elo_tier=elo_tier, patch=patch)
        if lane:
            query = query.filter_by(lane=lane)
        return query.all()

    def list_available_patches(self, elo_tier: str) -> list[str]:
        rows = (
    ...
    def get_history(
        self, champion_id: str, elo_tier: str, lane: str | None = None
    ...
    def get_one(self, champion_id: str, lane: str, elo_tier: str, patch: str) -> ChampionScore | None:
        return (
            self.db.query(ChampionScore)
            .filter_by(champion_id=champion_id, lane=lane, elo_tier=elo_tier, patch=patch)
            .one_or_none()
        )

    def get_latest_for_champion_lane(
        self, champion_id: str, lane: str, elo_tier: str
    ) -> ChampionScore | None:
        return (
            self.db.query(ChampionScore)
            .join(Patch, Patch.version_label == ChampionScore.patch)
    ...
    def list_latest_by_champion_lane_keys(
        self, keys: set[tuple[str, str]], elo_tier: str
