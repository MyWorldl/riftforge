...
def get_champion_explanation(
    ...
    result = score_service.get_explanation(
        db, champion_id, lane, elo_tier, patch, region
    )
    if result is None:
        raise HTTPException(
            status_code=404,
            detail="Sem score calculado pra esse campeão/rota/elo/patch.",
        )
    return result
...
def get_available_patches(
    ...
    return score_service.list_available_patches(db, elo_tier, region)
...
def get_champion_history(
    ...
    return score_service.get_champion_history(db, champion_id, elo_tier, lane, region)
